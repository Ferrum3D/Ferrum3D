...
namespace FE::Graphics
{
    ...
    void TextureAssetManager::AsyncIOCallback(const IO::AsyncBlockReadResult& result)
    {
        ...
        case LoadingStage::kMips:
            if (!OnMipChainLoaded(data, request, static_cast<uint32_t>(readRequest->m_userData1), result.m_blockIndex))
            {
                request->m_asset->m_status.store(AssetLoadingStatus::kFailed, std::memory_order_release);
                request->m_asset->m_completionWaitGroup->Signal();
                m_requestPool.Delete(request);
    ...
    bool TextureAssetManager::OnHeaderLoaded(const festd::span<const std::byte> data, Request* request, IO::IStream* stream)
    {
        ...
        for (uint32_t i = 0; i < request->m_mipChains.size(); ++i)
        {
            ...
            auto* mipChainData = static_cast<std::byte*>(Memory::DefaultAllocate(mipChainByteSize));
            mipRequest.m_data = mipChainData;

            if (reader.AvailableSpace() > 0)
            {
                // The least detailed mip chain might be in the same block as the header.
                if (!OnMipChainLoaded({ reader.m_ptr, reader.m_end }, request, i, 0))
                    return false;
    ...
    bool TextureAssetManager::OnMipChainLoaded(const festd::span<const std::byte> data, Request* request,
                                               const uint32_t mipChainIndex, const uint32_t blockIndex)
    {
        FE_PROFILER_ZONE();
        ...
        std::byte* mipChainData = mipRequest.m_data + Compression::kBlockSize * static_cast<uintptr_t>(blockIndex);
        memcpy(mipChainData, data.data(), data.size());
        ...
        copyCommandListBuilder.UploadTexture(request->m_asset->m_resource.Get(), mipRequest.m_data, 0, subresource);
