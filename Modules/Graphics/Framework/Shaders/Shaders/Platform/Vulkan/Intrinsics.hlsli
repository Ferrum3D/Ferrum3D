...
namespace Platform
{
    ...
    uint32_t BitFieldInsert(uint32_t base, uint32_t insert, uint32_t offset, uint32_t count);
...
namespace Bit
{
    ...
    {
        return Platform::BitFieldExtract(source, offset, size);
    }


    uint32_t FieldInsert(const uint32_t source, const uint32_t insert, const uint32_t offset, const uint32_t size)
    {
        return Platform::BitFieldInsert(source, insert, offset, size);
    }
} // namespace Bit
