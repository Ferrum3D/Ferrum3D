...
namespace FE::Graphics::Core
{
    struct GraphicsPipelineDesc final
    {
        ...
        {
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kVertex)].IsValid());
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kAmplification)].IsValid());
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kMesh)].IsValid());
            m_shaders[festd::to_underlying(ShaderStage::kVertex)] = shaderName;
            return *this;
        }

        GraphicsPipelineDesc& SetAmplificationShader(const Env::Name shaderName)
        {
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kAmplification)].IsValid());
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kVertex)].IsValid());
            FE_Assert(m_inputLayout.IsEmpty(), "Input layout is ignored for mesh shader pipelines");
            m_shaders[festd::to_underlying(ShaderStage::kAmplification)] = shaderName;
            return *this;
        }

        GraphicsPipelineDesc& SetMeshShader(const Env::Name shaderName)
        {
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kMesh)].IsValid());
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kVertex)].IsValid());
            FE_Assert(m_inputLayout.IsEmpty(), "Input layout is ignored for mesh shader pipelines");
            m_shaders[festd::to_underlying(ShaderStage::kMesh)] = shaderName;
            return *this;
        }

        GraphicsPipelineDesc& SetPixelShader(const Env::Name shaderName)
        {
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kPixel)].IsValid());
        ...
        {
            FE_Assert(!m_shaders[festd::to_underlying(ShaderStage::kAmplification)].IsValid()
                          && !m_shaders[festd::to_underlying(ShaderStage::kMesh)].IsValid(),
                      "Input layout is ignored for mesh shader pipelines");
    ...
    struct GraphicsPipeline : public PipelineBase
    {
        ...
        [[nodiscard]] bool IsMeshShaderPipeline() const
        {
            return m_desc.m_shaders[festd::to_underlying(ShaderStage::kMesh)].IsValid();
        }
