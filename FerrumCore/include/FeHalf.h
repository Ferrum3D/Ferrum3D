...
namespace Ferrum
{
	...
	union HalfFormat
	{
		...
		constexpr HalfFormat(uint16_t b) : Data(b) { }
	};
	...
	*/
	struct FeHalf
	{
		constexpr FeHalf() noexcept : m_data(0) { }

		constexpr FeHalf(int iVal) noexcept : FeHalf(float(iVal)) { }

		constexpr FeHalf(float fVal) noexcept : m_data(0) {
			FloatFormat f{};
			f.Data = fVal;
			...
			// A normal number (not infinity, zero or NaN)
				...
				if (exp < -24) { // Zero in half precision
					m_data.SEM.M = 0;
					m_data.SEM.E = 0;
				}
				else if (exp < -14) { // Denorm
					m_data.SEM.E = 0;
					...
					case 0:
						m_data.SEM.M = 0;
						break;
					case 1:  m_data.SEM.M = 512 + (f.SEM.M >> 14);	break;
					case 2:  m_data.SEM.M = 256 + (f.SEM.M >> 15);	break;
					case 3:  m_data.SEM.M = 128 + (f.SEM.M >> 16);	break;
					case 4:  m_data.SEM.M = 64  + (f.SEM.M >> 17);	break;
					case 5:  m_data.SEM.M = 32  + (f.SEM.M >> 18);	break;
					case 6:  m_data.SEM.M = 16  + (f.SEM.M >> 19);	break;
					case 7:  m_data.SEM.M = 8   + (f.SEM.M >> 20);	break;
					case 8:  m_data.SEM.M = 4   + (f.SEM.M >> 21);	break;
					case 9:  m_data.SEM.M = 2   + (f.SEM.M >> 22);	break;
					case 10: m_data.SEM.M = 1;						break;
					default:										break;
					}
				}
				else if (exp > 15) { // Infinity in half precision
					m_data.SEM.M = 0;
					m_data.SEM.E = FeHalfConst::MaxExpVal;
				}
				else {
		...
		constexpr FeHalf(double dVal) noexcept : FeHalf(float(dVal)) { }

		constexpr FeHalf(const FeHalf& other) noexcept : m_data(other.m_data) { }
		constexpr FeHalf(FeHalf&& other) noexcept : m_data(other.m_data) { }
		...
		constexpr bool operator!=(FeHalf other) const noexcept {
			...
			if ((m_data.Data & 0x7f'ff) == 0 && (other
				.m_data.Data & 0x7f'ff) == 0) {
				return false;
			}
	...
	private:
		...
		constexpr FeHalf(uint16_t s, uint16_t e, uint16_t m) : m_data(0) {
			m_data.SEM.S = s;
			m_data.SEM.E = e;
			m_data.SEM.M = m;
		}
