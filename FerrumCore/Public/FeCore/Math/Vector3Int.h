...
namespace FE
{
    ...
    struct PackedVector3Int final
    {
        ...
        explicit FE_FORCE_INLINE FE_NO_SECURITY_COOKIE PackedVector3Int(const Vector3Int value)
            : x(value.x)
            , y(value.y)
            , z(value.z)
        {
        ...
        explicit FE_FORCE_INLINE FE_NO_SECURITY_COOKIE operator Vector3Int() const
        {
            return { x, y, z };
        }
    };
