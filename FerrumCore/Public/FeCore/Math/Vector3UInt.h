...
namespace FE
{
    ...
    struct PackedVector3UInt final
    {
        ...
        explicit FE_FORCE_INLINE FE_NO_SECURITY_COOKIE PackedVector3UInt(const Vector3UInt value)
            : x(value.x)
            , y(value.y)
            , z(value.z)
        {
        ...
        explicit FE_FORCE_INLINE FE_NO_SECURITY_COOKIE operator Vector3UInt() const
        {
            return { x, y, z };
        }
    };
