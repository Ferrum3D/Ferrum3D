...
namespace FE::Memory
{
    struct BuddyAllocator final
    {
        ...
        explicit BuddyAllocator(uint32_t arenaByteSize);
    ...
    private:
        ...
        [[nodiscard]] static uint32_t BlockSizeForOrder(uint32_t order);
        [[nodiscard]] uint32_t OrderForBlockSize(uint32_t blockSize) const;
    };
} // namespace FE::Memory
