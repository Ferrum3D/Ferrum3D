...
namespace Ferrum
{
	void FeJobSystem::Init(const std::vector<FeJobType>& threadFlagArray) {
		...
		for (size_t i = 0; i < threadFlagArray.size(); ++i) {
			...
			desc.Id = i;
			desc.Name = FeFormatString("Worker thread #{}", i);
	...
	FeJobHandle FeJobSystem::ScheduleJob(FeJobFunction job, FeJobType type, void* inst) {
		FeJobHandle handle(job, type, inst);
