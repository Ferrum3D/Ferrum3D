...
namespace FE::Threading
{
    ...
    {
        const auto result = SleepConditionVariableCS(reinterpret_cast<PCONDITION_VARIABLE>(&m_nativeConditionVariable),
                                                     reinterpret_cast<LPCRITICAL_SECTION>(lock.mutex()->m_nativeMutex),
                                                     INFINITE);
        FE_Verify(result);
    }
