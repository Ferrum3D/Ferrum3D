...
namespace FE::IO
{
    ...
    ResidencyTicket AssetManager::LoadAsset(const AssetID assetId)
    {
        ...
        completionWaitGroup = graph.Dispatch("DeserializeArtifact", { completionWaitGroup }, [&slot, artifactMetaLocation] {
            ...
            {
                slot.m_metadataError.m_code = ArtifactMetadataErrorCode::kIoError;
        ...
        graph.Dispatch("InitialRead", { completionWaitGroup }, [&slot] {
            ...
            const Rc<Async::IController> initialReadController = Async::Read(initialReadBatch);
            ...
            if (initialReadController->GetStatus() != Async::Status::kSucceeded)
            {
                slot.m_metadataError.m_code = ArtifactMetadataErrorCode::kIoError;
                slot.m_metadataError.m_assetId = slot.m_slot.m_assetId;
                slot.m_metadataError.m_source = payloadRecord.m_resolvedDataSource.m_filePath;
                slot.m_metadataError.m_message = "payload read failed";
                slot.m_metadataValid = false;
                slot.m_readBuffer.clear();
                return;
            }
