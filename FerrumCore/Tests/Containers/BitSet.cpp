...
static BitSetWord* AllocateGuardedWords(size_t count)
{
...
static void FreeGuardedWords(BitSetWord* words, size_t count)
{
    const size_t pageSize = Memory::GetPlatformSpec().Granularity;
    const size_t byteSize = AlignUp(count * sizeof(BitSetWord), pageSize);
    const size_t totalWords = byteSize / sizeof(BitSetWord);
...
TEST(DynamicBitSet, Traverse)
{
    ...
    const uint32_t iterationCount = 10000;
    const uint32_t arraySize = 50000;
    const uint32_t maxSetBits = 64;
    ...
    for (uint32_t i = 0; i < iterationCount; ++i)
    {
        ...
        eastl::sort(setIndices.begin(), setIndices.end());
