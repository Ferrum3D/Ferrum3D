...
namespace FE::Osmium
{
    extern "C"
    {
        ...
        FE_DLL_EXPORT IImage* ITransientResourceHeap_CreateImage(
            ITransientResourceHeap * self, TransientImageDesc * desc, TransientResourceAllocationStats * stats)
        {
            return self->CreateImage(*desc, *stats).Detach();
        }

        FE_DLL_EXPORT IBuffer* ITransientResourceHeap_CreateBuffer(
            ITransientResourceHeap * self, TransientBufferDesc * desc, TransientResourceAllocationStats * stats)
        {
